...
public class AboutProgramValidation
{
	...
	private AboutProgramLocator aboutProgramLocator;
	...
	String getMetaHost;
	WebDriver driver;
	public AboutProgramValidation(WebDriver driver, String sheetName, ArrayList<ArrayList<String>> rows)
	{
		this.SHEET_NAME = sheetName;
		this.ROWS = rows;
		this.driver = driver;
		this.aboutProgramLocator = new AboutProgramLocator(driver);
	...
	public String executeProcess(String process, ArrayList<String> row) {
		...
		try {
			...
			case "ValidateFAQ":
				...
				{
					faqFromValidator = aboutProgramLocator.getFAQFromValidator();
				}
	...
	private void environment(String environmentFromExcel)
	{
		try
		{
			String checkEnvironment = aboutProgramLocator.setEnvironment(environmentFromExcel);
			getMetaHost = aboutProgramLocator.setMetaHostURL();
			//getImageHost = aboutProgramLocator.setImageEndpoint(environmentFromExcel);
		}
		catch(Exception e)
		{
			e.printStackTrace();
	...
	private void courseCode(String courseCodeFromExcel) {
		...
		try {
			String checkCourseCode = aboutProgramLocator.getCourseCodeText(courseCodeFromExcel);
	...
	private void courseTitle(String courseTitleFromExcel)
	{
		...
		try
		{
			String checkCourseTitleStatus = aboutProgramLocator.getCourseTitleText(courseTitleFromExcel);
	...
	private void courseOrganization(String courseOrganizationFromExcel) {
		...
		try
		{
			String checkCourseOrgStatus = aboutProgramLocator.getCourseOrganizationFormatText(courseOrganizationFromExcel);
	...
	private void courseDescription(String courseDescriptionFromExcel) {
		...
		try {
			String checkCourseDes = aboutProgramLocator.getCourseDescription(courseDescriptionFromExcel);
	...
	private void courseType1(String courseType1FromExcel) {
		...
		try {
			String checkCourseType1Status = aboutProgramLocator.getCourseType1(courseType1FromExcel);
	...
	private void courseType2(String courseType2FromExcel) {
		...
		try {
			String checkCourseType = aboutProgramLocator.getCourseType2(courseType2FromExcel);
	...
	private void courseLevel(String courseLevelFromExcel) {
		...
		try {
			String checkCourseLevel = aboutProgramLocator.getCourseLevel(courseLevelFromExcel);
	...
	private void courseInformation(ArrayList<String> infoValues)
	{
		...
		try 
		{
			ArrayList<String> infoContentFromCourse = aboutProgramLocator.getInfoContentFromCourse(infoHeadingFromExcel);
	...
	private void courseOutline()
	{
		try
		{
			aboutProgramLocator.processCourseOutLineSection();
		} 
		catch (Exception e)
		{
			sheetStatus = "Fail";
	...
	private void typeofCertificate(String typeOfCertificateFromExcel)
	{
		...
		try
		{
			String checkEarnYourCertificate = aboutProgramLocator.getTypeofCertificate(typeOfCertificateFromExcel);
	...
	private void aboutCourse(String aboutCourseFromExcel)
	{
		...
		try
		{
			String checkAbout = aboutProgramLocator.getAboutCourse(aboutCourseFromExcel);
	...
	private void includes(String includesFromExcel)
	{
		...
		try
		{
			String checkIncludes = aboutProgramLocator.getIncludes(includesFromExcel);
	...
	private void create(String createFromExcel)
	{
		...
		try
		{
			String checkCreate = aboutProgramLocator.getCreate(createFromExcel);
	...
	private void exercisesToExplore(String exerciseFromExcel)
	{
		...
		try 
		{
			String checkExerciseToExplore = aboutProgramLocator.getExerciseToExplore(exerciseFromExcel);
	...
	{
		if(!expertsFromExcel.contains("NA"))
		{
			
			HashMap<String, HashMap<String, String>> experts = aboutProgramLocator.getExperts();
	...
	private void duration(String durationFromExcel)
	{
		...
		try {
			String checkDurationDetails = aboutProgramLocator.getDurationInfo(durationFromExcel);
	...
	private void startsOn(String startsOnFromExcel)
	{
		...
		try
		{
			String checkStartsOnDetails = aboutProgramLocator.getStartsOn(startsOnFromExcel);
	...
	private void flatPriceINRWithoutGST(String flatPriceWithoutGSTFromExcel) {
		...
		try {
			String checkFlatPrice = aboutProgramLocator.getflatPrice(flatPriceWithoutGSTFromExcel);
	...
	private void flatPriceUSD(String priceUSDFromExcel) {
		...
		try {
			String checkUSDPriceStatus = aboutProgramLocator.getUSDPrice(priceUSDFromExcel);
	...
	private void category(ArrayList<String> categoryName)
	{
		...
		try {
			ArrayList<String> verifyCategory = aboutProgramLocator.checkCategory(categoryName);
	...
	private void launchUrlAndTestRedirection(String currentURL, String redirectURLFromExcel) 
	{
		try
		{
			String redirectedURL = aboutProgramLocator.launchCourseURL(redirectURLFromExcel);
	...
	private void canonical(String canonicalURL)
	{
		...
		try
		{
			String checkCanonicalURL = aboutProgramLocator.getCanonicalURL(canonicalURL);
	...
	private void checkMetaTagContentByName(ArrayList<String> tags) 
	{
		for(int i = 0; i < tags.size(); i++)
		{
			...
			try
			{
				String content = aboutProgramLocator.getAttributeOfTag("meta[name='" + attributes.get("name") + "']", "content").trim();
	...
	private void checkMetaTagContentByProperty(ArrayList<String> tags)
	{
		for (int i = 0; i < tags.size(); i++)
		{
			...
			try
			{
				...
				String content = aboutProgramLocator.getAttributeOfTag("meta[property='" + attributes.get("property") + "']", "content");
	...
	private void checkTagWithInnerText(ArrayList<String> tags) {
		for (int i = 0; i < tags.size(); i++)
		{
			...
			try {
				aboutProgramLocator.getTagWithInnerText(tagAndText[0], tagAndText[1]);
			} catch (Exception e) {
				sheetStatus = "Fail";
				markColumnFailed(i);
			}
		...
		for (int i = 0; i < tags.size(); i++)
		{
			...
			try
			{
				if(tag.equalsIgnoreCase("checkImgSrcAndAlt"))
					continue;
				HashMap<String, String> attributes = extractAttributesFromString(tag);
				String altText = aboutProgramLocator.getAttributeOfTag("img[src='" + attributes.get("src") + "']", "alt");
	...
	private void faq(ArrayList<String> faqRow)
	{
		...
		try
		{
			ArrayList<String> answersFromParagraphs = aboutProgramLocator.getAnswersForFAQQuestion(question);
	...
	{
		aboutProgramLocator.launchValidator();
		try
		{
			String headingStatus = aboutProgramLocator.validateHeading(row.get(1));
	...
	{
		String[] status = aboutProgramLocator.checkRedirectStatus(url, statusCode1, statusCode2);
