...
public class GLXValidation implements Callable<String>
{
	...
	public void CTADownloadDetails()
	{
		...
		{
			sheetStatus = "Fail";
			RegressionTesting.EXCEL_DATA_AS_SHEEET_NAME_AND_ROWS_MAP.get("GLX").get(3).set(0, "CTADownloadDetails - failed");
		}
	}
	public void programs(ArrayList<String> data)
	{
		if(!data.contains("NA"))
		{
			...
			if(getStatus.size()>0)
			{
				...
				{
					sheetStatus = "Fail";
					RegressionTesting.EXCEL_DATA_AS_SHEEET_NAME_AND_ROWS_MAP.get("GLX").get(4).add(i+1, (getStatus.get(i) + "programs - failed"));
				}
			}
		}
	}
	public void ExploreCourse()
	{
		...
		if(getStatus.size()>0)
		{
			...
			{
				sheetStatus = "Fail";
				RegressionTesting.EXCEL_DATA_AS_SHEEET_NAME_AND_ROWS_MAP.get("GLX").get(5).add(i+1, (getStatus.get(i) + "programs - failed"));
			}
		}
	}
	...
	public String call() throws Exception {
		...
		for(int i = 0; i < this.sheetData.size(); i++)
		{
			...
			{
			/*
			 * case "facebook": facebook(); break; case "instagram": instagram(); break;
