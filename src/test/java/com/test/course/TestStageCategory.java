...
public class TestStageCategory
{

	ITestResult result;
	WebDriverWait wait;
	StagecategoryLocator stagecategoryLocator;
	...
	public TestStageCategory(String excelPath) 
	{
	...
	public void aboutStageCategoryPage(String excelPath)
	{
		...
		try {
			...
			for (Entry<String, ArrayList<ArrayList<String>>> entry : data.entrySet()) {
				...
				try {
					StagecategoryValidator stagecategoryValidator = new StagecategoryValidator(sheetName, sheetData);
					String sheetStatus = stagecategoryValidator.processSheetData();
