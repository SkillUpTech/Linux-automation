...
public class TestCategoryUI 
{
	ITestResult result;
	WebDriverWait wait;
	CategoryUILocator categoryUILocator;
	...
	public TestCategoryUI(String excelPath) 
	{
		softAssert = new SoftAssert();
		categoryUILocator = new CategoryUILocator();
		excelPath = "D:\\SkillUp_Testing\\SEO\\Category\\categoryName.xlsx";
		this.aboutCategoryUI(excelPath);
	}
	
	public void aboutCategoryUI(String excelPath)
	{
		...
		try {
			...
			for (Entry<String, ArrayList<ArrayList<String>>> entry : data.entrySet()) {
				...
				try {
					CategoryUIValidator categoryUIValidator = new CategoryUIValidator(sheetName, sheetData);
					String sheetStatus = categoryUIValidator.processSheetData();
