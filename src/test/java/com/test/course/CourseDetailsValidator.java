...
public class CourseDetailsValidator
{
	...
	public CourseDetailsValidator(String sheetName, ArrayList<ArrayList<String>> rows)
	{
		...
		try
		{
			switch (process.trim()) 
			{
				...
				case "validateFAQ":
					...
					{
						faqFromValidator = courseDetails.getFAQFromValidator();
					}
	...
	private void checkMetaTagContentByProperty(ArrayList<String> tags)
	{
		for (int i = 0; i < tags.size(); i++)
		{
			...
			try
			{
				...
				String content = courseDetails.getAttributeOfTag("meta[property='" + attributes.get("property") + "']", "content");
	...
	private void checkTagWithInnerText(ArrayList<String> tags) {
		for (int i = 0; i < tags.size(); i++)
		{
			...
			try {
				courseDetails.getTagWithInnerText(tagAndText[0], tagAndText[1]);
			} catch (Exception e) {
				markColumnFailed(i);
			}
		}
	...
	private void faq(ArrayList<String> faqRow)
	{
		...
		try
		{
			ArrayList<String> answersFromParagraphs = courseDetails.getAnswersForFAQQuestion(question);
	...
	{
		courseDetails.launchValidator();
		try
		{
			String headingStatus = courseDetails.validateHeading(row.get(1));
	...
	{
		String[] status = courseDetails.checkRedirectStatus(url, statusCode1, statusCode2);
	...
	{
		String cellValue = TestCourseDetails.EXCEL_DATA_AS_SHEEET_NAME_AND_ROWS_MAP.get(SHEET_NAME).get(CURRENT_ROW).get(columnIndex);
		TestCourseDetails.EXCEL_DATA_AS_SHEEET_NAME_AND_ROWS_MAP.get(SHEET_NAME).get(CURRENT_ROW).set(columnIndex, (cellValue + " - failed"));
	...
	private void collectSheetResult()
	{
		...
		TestCourseDetails.EXCEL_DATA_AS_SHEEET_NAME_AND_ROWS_MAP.get(SHEET_NAME).add(startTimeRow);
