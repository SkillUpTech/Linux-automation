...
public class TestSEOGeneric
{
	ITestResult result;
	WebDriverWait wait;
	SEOGenericLocator seoGenericLocator;
	...
	public TestSEOGeneric(String excelPath)
	{
		softAssert = new SoftAssert();
		seoGenericLocator = new SEOGenericLocator();
		excelPath = "D:\\SkillUp_Testing\\SEO\\AboutCourse\\GenericFunction.xlsx";
		this.seoGenericLocator(excelPath);
	}

	public void seoGenericLocator(String excelPath)
	{
		...
		try 
		{
			...
			for (Entry<String, ArrayList<ArrayList<String>>> entry : data.entrySet())
			{
				...
				try
				{
					SEOGenericValidator seoGenericValidator = new SEOGenericValidator(sheetName, sheetData);
					String sheetStatus = seoGenericValidator.processSheetData();
