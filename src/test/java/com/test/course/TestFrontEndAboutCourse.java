...
public class TestFrontEndAboutCourse 
{
	ITestResult result;
	WebDriverWait wait;
	FrontEndAboutPageLocator frontEndAboutPageLocator;
	...
	public TestFrontEndAboutCourse(String excelPath)
	{
		softAssert = new SoftAssert();
		frontEndAboutPageLocator = new FrontEndAboutPageLocator();
		excelPath = "D:\\SkillUp_Testing\\SEO\\AboutCourse\\AboutPageFrontEnd.xlsx";
		this.frontEndAboutPage(excelPath);
	}

	public void frontEndAboutPage(String excelPath)
	{
		...
		try {
			...
			for (Entry<String, ArrayList<ArrayList<String>>> entry : data.entrySet()) {
				...
				try {
					FrontEndAboutPageValidator frontEndAboutPageValidator = new FrontEndAboutPageValidator(sheetName, sheetData);
					String sheetStatus = frontEndAboutPageValidator.processSheetData();
